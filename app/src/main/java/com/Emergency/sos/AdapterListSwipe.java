...
public class AdapterListSwipe extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements SwipeItemTouchHelper.SwipeHelperAdapter {
    ...
    private List<Contact> items_swiped = new ArrayList<>();
    ...
    public AdapterListSwipe(Context context, List<Contact> items) {
        this.items = items;
        ctx = context;
    }
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        if (holder instanceof OriginalViewHolder) {
            ...
            final Contact p = items.get(position);
    ...
    public void onAttachedToRecyclerView(RecyclerView recyclerView) {
        recyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                for (Contact s : items_swiped) {
                    int index_removed = items.indexOf(s);
                    if (index_removed != -1) {
                        Contacts.allinfo.remove(s);
                        SOS.numbers.remove(s.phone);
//                        items.remove(index_removed);
                            Contacts.mAdapter.notifyDataSetChanged();
                        notifyItemRemoved(index_removed);
                        SOS.arraylist.putListObject("getinfo",Contacts.allinfo);
    ...
    public void onItemDismiss(int position) {
        ...
        if (items.get(position).swiped) {
            Contacts.allinfo.remove(position);
            SOS.numbers.remove(position);
            Contacts.mAdapter.notifyDataSetChanged();
